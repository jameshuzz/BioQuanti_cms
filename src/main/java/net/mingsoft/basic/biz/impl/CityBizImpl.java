		...
		if("tree".equals(type)){
			...
			if(level >= 1){//树类型
				...
				for (Long provinceKey : province.keySet()) {
					...
					provinceBean.setChildren(new ArrayList<CityBean>());
					if(level >= 2){
						//遍历市级数据，组织第二级
						...
						for(Long cityKey : cityMap.keySet()){
							...
							cityBean.setChildren(new ArrayList<CityBean>());
							if(level >= 3){
								//遍历县级数据，组织第三极
								...
								for(Long countyKey : countyMap.keySet()){
									...
									countyBean.setChildren(new ArrayList<CityBean>());
									if(level >= 4){
										//遍历镇数据，组织第四级
										...
										for(Long townKey : townMap.keySet()){
											...
											townBean.setChildren(new ArrayList<CityBean>());
											if(level >= 5){
												//遍历村数据，组织第五级
												...
												for(Long villageKey : villageMap.keySet()){
													...
													townBean.getChildren().add(villageBean);
												}
											}
											if(StringUtils.isNotBlank(townBean.getName())) {
												countyBean.getChildren().add(townBean);
											}

										}
									}
									if(StringUtils.isNotBlank(countyBean.getName())) {
										cityBean.getChildren().add(countyBean);
									}
								}
							}
							if(StringUtils.isNotBlank(cityBean.getName())) {
								provinceBean.getChildren().add(cityBean);
							}
