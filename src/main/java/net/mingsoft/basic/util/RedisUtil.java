...
public class RedisUtil {
	...
	private static RedisTemplate<String, Object> getRedisTemplate() {
		if(redis==null) {
			redis = (RedisTemplate<String, Object>) SpringUtil.getBean("redisTemplate");
		}
		return redis;
	}
	...
	public static void multiSet(Map map){
		RedisUtil.getValueOperations().multiSet(map);
	}


	public static List multiGet(Collection keys){
		return RedisUtil.getValueOperations().multiGet(keys);
	}


	/**
	 * 批量设置key-value值
	 * 按照spring cache默认序列化方式处理
	 * 		key 字符串序列化
	 * 		value jdk序列化
	 * @param map key redis存储的key
	 *            value redis存储的值
	 */
	public static void pipelineMultiSet(Map map){
		...
		redisTemplate.executePipelined((RedisCallback<Object>) conn -> {
			...
			for (Object o : map.entrySet()) {
				...
				redisStringCommands.set(serializedKey, serializedValue);
			}
			return null;
		});
