	...
	<el-header class="ms-header" height="50px">
		<el-col :span=2>
			<el-upload
				...
				<el-tooltip effect="dark" content="建议上传5M以下htm/html/css/js/jpg/gif/png/swf文件，同名文件会直接覆盖" placement="left-end">
					<@shiro.hasPermission name="basic:template:upload">
						<el-button size="default" class="el-icon-upload2" type="primary">上传模板文件</el-button>
					</@shiro.hasPermission>
				</el-tooltip>
			</el-upload>
		</el-col>
		<el-col :span=22>
			<el-button size="default" plain onclick="" style="float: right" @click="back"><i class="iconfont icon-fanhui"></i>返回</el-button>
			<@shiro.hasPermission name="basic:template:update">
				<el-button :disabled="form.name == ''" style="float: right;margin-right: 10px" type="primary" class="iconfont icon-baocun" size="default" @click="saveFile">保存</el-button>
			</@shiro.hasPermission>
		</el-col>
	</el-header>
	<el-main class="ms-container" style="flex-direction: row" v-loading="loading">
		...
		<div :class="imgFlag?'file-img':'file-edit'">
			<el-main style="padding: 0px;">
				<codemirror ref="code" v-if="!imgFlag && fileFlag" v-model:value="form.fileContent" :options="cmOption" :on-success="codemirrorSuccess">
				</codemirror>
				<img :src="imgPath" v-else-if="imgFlag" style="max-width: 100%;">
				<el-empty description="请选择模板文件" v-else></el-empty>
			</el-main>
		</div>
			...
			view: function (row) {
				...
				switch (type) {
					...
					case "文件":
						...
						break;
					case "图片":
						this.imgPath = ms.base+"/" + this.templateData.uploadPath + "/" + row.name;
						this.imgFlag = true;
						this.loading = false;
						break;
					default:
