...
<div id="form" v-cloak>
    ...
    <el-main class="ms-container">
        <el-scrollbar class="ms-scrollbar" style="height: 100%;">
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" size="default">
                <el-row
                        :gutter=0
...
<script>
    var form = new _Vue({
        ...
        data: function () {
            // 表单rule校验 不允许空格
            var checkBlank = function (rule, value, callback){
                if (value && value.indexOf(' ')!==-1 ){
                    callback(new Error('不允许含有空格符'));
                }else {
                    callback();
                }
            }
        ...
        methods: {
            back: function () {
                ms.util.openSystemUrl("/mdiy/dict/data/index.do?dictType=" + this.form.dictType,true);
            },
            save: function () {
                var that = this;
                    ...
                    if (valid) {
                        ...
                        ms.http.post(url, data).then(function (data) {
                            if (data.result) {
                                ...
                                ms.util.openSystemUrl("/mdiy/dict/data/index.do?dictType=" + that.form.dictType,that.form.id > 0);
                            } else {
                                that.$notify({
                                    title: '失败',
                                    message: data.msg,
